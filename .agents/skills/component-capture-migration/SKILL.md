...
Read [references/project-map.md](references/project-map.md) at the start of each task, then inspect the current repo files it names because catalog and Portal wiring may have changed.
...
- **Composition pattern**: build the target from existing components and add a Portal pattern rather than a new primitive.
...
- Documentation/Portal/test gaps.
...
- Update `packages/ui/docs/components.md` with a link to the Portal route and when-to-use guidance.
- Add or update deterministic fixtures in `apps/ui-portal/src/client/fixtures/` when examples need realistic data.
- Add the Portal page under `apps/ui-portal/src/client/pages/core/components/<slug>.page.tsx`.
- Register the page in the matching `apps/ui-portal/src/client/registry/entries/*.tsx` file. The active registry is Core-only; do not register Agent components or patterns.
...
- Start Lemn UI with `pnpm dev:portal` or rely on Playwright's configured web server.
...
- E2E tests in `apps/ui-portal/tests/e2e/` when behavior requires a browser.
- Visual coverage by adding the route to `apps/ui-portal/tests/e2e/visual.e2e.ts` when the component has meaningful visual states or becomes a primary catalog surface.
