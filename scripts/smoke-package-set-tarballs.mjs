...
const requiredPublicExports = new Set([
	".",
	"./tokens",
	"./catalog",
	"./blocks",
...
async function collectRelativeFiles(directory, outputRoot = directory) {
	...
	for (const entry of entries) {
		...
		if (entry.isFile())
			files.push(relative(outputRoot, child).replaceAll("\\", "/"));
	}
...
function collectExportTargets(definition) {
	...
	if (Array.isArray(definition))
		return definition.flatMap(collectExportTargets);
...
try {
	...
	const tarballPath = join(
		temporaryRoot,
		`lemn-ltd-ui-${uiPackage.version}.tgz`,
	);
	const brandRuntimeTarballPath = join(
		temporaryRoot,
		`lemn-ltd-brand-runtime-${brandRuntimePackage.version}.tgz`,
	);
	...
	const brandRuntimePackResult = JSON.parse(
		run(pnpmCommand, [
			"--config.verify-deps-before-run=false",
			"--filter",
	...
	assert(
		brandRuntimePackResult.name === "@lemn-ltd/brand-runtime" &&
			brandRuntimePackResult.version === brandRuntimePackage.version,
		"Packed brand-runtime identity must match its manifest",
	);
	assert(
	...
	for (const [name, result, requiredFiles] of [
		[
			"@lemn-ltd/brand-contract",
			brandContractPackResult,
			["CHANGELOG.md", "LICENSE", "README.md"],
		],
		[canonicalPackageName, packResult, ["CHANGELOG.md", "README.md"]],
		[
			"@lemn-ltd/brand-runtime",
			brandRuntimePackResult,
			["CHANGELOG.md", "LICENSE", "README.md"],
		],
		[
			"@lemn-ltd/brand-studio",
			brandStudioPackResult,
			["CHANGELOG.md", "LICENSE", "README.md"],
		],
	]) {
		...
		for (const requiredFile of requiredFiles) {
			assert(
				paths.has(requiredFile),
				`${name} tarball is missing required ${requiredFile}`,
			);
		}
	}

	assert(
		packResult.name === canonicalPackageName,
		`Packed name must be ${canonicalPackageName}`,
	);
	assert(
		packResult.version === uiPackage.version,
		"Packed version must match packages/ui/package.json",
	);
	...
	const isolatedBrandingSmoke = JSON.parse(
		run(
			process.execPath,
			[
				"--input-type=module",
          ...
          const { systemBrandingTemplates } = await import('@lemn-ltd/brand-contract/system-brandings');
          const runtime = await import('@lemn-ltd/brand-runtime');
          const server = await import('@lemn-ltd/brand-runtime/server');
          const template = systemBrandingTemplates.find((entry) => entry.id === 'aster-vault');
          console.log(JSON.stringify({
            template: template && [template.id, template.version, template.definition.defaultModeId],
            runtime: typeof runtime.resolveBranding,
            server: typeof server.createBrandingSsrParts,
            reactResolved,
            uiResolved,
          }));
        `,
	...
	assert(
		isolatedBrandingSmoke.template?.[0] === "aster-vault" &&
			isolatedBrandingSmoke.template?.[1] === 1 &&
			isolatedBrandingSmoke.template?.[2] === "light" &&
			isolatedBrandingSmoke.runtime === "function" &&
			isolatedBrandingSmoke.server === "function" &&
			isolatedBrandingSmoke.reactResolved === false &&
			isolatedBrandingSmoke.uiResolved === false,
		"System branding catalog and server runtime must load without React, @lemn-ltd/ui, or CSS evaluation",
	);

	const files = packResult.files ?? [];
	const packedPaths = new Set(files.map((file) => file.path));
	const packedSourcePaths = files
		.filter((file) => file.path.startsWith("src/"))
		.map((file) => file.path);
	assert(
		packedSourcePaths.length === 0,
		`Tarball must not contain src/: ${packedSourcePaths.join(", ")}`,
	);
	const publicExportKeys = Object.keys(uiPackage.exports ?? {});
	for (const exportKey of requiredPublicExports) {
		assert(
			publicExportKeys.includes(exportKey),
			`Package manifest must expose ${exportKey}`,
		);
	}
	for (const exportKey of publicExportKeys) {
		for (const target of collectExportTargets(uiPackage.exports[exportKey])) {
			assert(
				target.startsWith("./"),
				`Export ${exportKey} target must be package-relative: ${target}`,
			);
			assert(
				packedPaths.has(target.slice(2)),
				`Tarball is missing export ${exportKey} target ${target}`,
			);
		}
	}

	const expectedCssPaths = (
		await collectCssFiles(join(root, "packages/ui/src"))
	).sort();
	...
	const unexpectedCss = packedCssPaths.filter(
		(path) => !expectedCssPaths.includes(path),
	);
	assert(
	...
	for (const artifact of [
		...
		assert(
			...
			`UI tarball is missing provider release artifact ${artifact}`,
		);
	}
	...
	const installedPackage = JSON.parse(
		await readFile(
			join(consumerDirectory, "node_modules/@lemn-ltd/ui/package.json"),
			"utf8",
		),
	);
	...
	const installedBrandContract = JSON.parse(
		await readFile(
			join(
				consumerDirectory,
				"node_modules/@lemn-ltd/brand-contract/package.json",
			),
			"utf8",
		),
	);
	const installedBrandRuntime = JSON.parse(
		await readFile(
			join(
				consumerDirectory,
				"node_modules/@lemn-ltd/brand-runtime/package.json",
			),
			"utf8",
		),
	);
	const installedBrandStudio = JSON.parse(
		await readFile(
			join(
				consumerDirectory,
				"node_modules/@lemn-ltd/brand-studio/package.json",
			),
			"utf8",
		),
	);
	...
	assert(
		installedBrandRuntime.version === brandRuntimePackage.version,
		"Installed brand-runtime version must match its tarball",
	);
	assert(
	...
	assert(
		installedBrandRuntime.dependencies?.["@lemn-ltd/brand-contract"] ===
			brandContractPackage.version,
		`Packed Brand Runtime must depend on exact @lemn-ltd/brand-contract ${brandContractPackage.version}`,
	);
	assert(
		installedBrandStudio.dependencies?.["@lemn-ltd/brand-contract"] ===
			brandContractPackage.version,
		`Packed Brand Studio must depend on exact @lemn-ltd/brand-contract ${brandContractPackage.version}`,
	);
	assert(
		installedBrandStudio.peerDependencies?.["@lemn-ltd/ui"] ===
			uiPackage.version,
		`Packed Brand Studio must peer-depend on exact @lemn-ltd/ui ${uiPackage.version}`,
	);
	const catalogSmoke = JSON.parse(
		run(
			process.execPath,
			[
				"--input-type=module",
				"--eval",
				`const { systemBrandingTemplates } = await import('@lemn-ltd/brand-contract/system-brandings'); const template = systemBrandingTemplates.find((entry) => entry.id === 'aster-vault'); console.log(JSON.stringify({ id: template?.id, version: template?.version, modes: Object.keys(template?.definition.modes ?? {}) }));`,
			],
			consumerDirectory,
		),
	);
	assert(
		catalogSmoke.id === "aster-vault" &&
			catalogSmoke.version === 1 &&
			catalogSmoke.modes.includes("light") &&
			catalogSmoke.modes.includes("dark"),
		"System brandings subpath must execute in plain Node without UI or CSS side effects",
	);
	assert(
		installedPackage.name === canonicalPackageName,
		"Installed package must retain its canonical name",
	);
	assert(
		installedPackage.version === uiPackage.version,
		"Installed package version must match the tarball",
	);
	assert(
	...
	const installedReleasePackages = [
		installedBrandContract,
		installedPackage,
		installedBrandRuntime,
		installedBrandStudio,
	];
	const unresolvedCatalogEntries = installedReleasePackages.flatMap(
		(manifest) =>
			dependencyGroups.flatMap((group) =>
				Object.entries(manifest[group] ?? {})
					.filter(([, version]) => !exactPublishedVersion.test(String(version)))
					.map(
						([name, version]) => `${manifest.name}:${group}.${name}=${version}`,
					),
			),
	);
	assert(
	...
	const npmList = JSON.parse(
		run(
			npmCommand,
			["ls", canonicalPackageName, "--depth=0", "--json"],
			consumerDirectory,
		),
	);
	const listedPackage = npmList.dependencies?.[canonicalPackageName];
	assert(
		listedPackage?.version === uiPackage.version,
		"npm must list the canonical package directly",
	);
	assert(
		!listedPackage.invalid,
		"npm must not classify the canonical package as invalid",
	);
	assert(
		!npmList.problems?.length,
		`npm reported package problems: ${npmList.problems?.join(", ")}`,
	);
	...
	for (const [packageName, expectedVersion] of Object.entries(
		...
		assert(
			...
			`${packageName} must resolve to ${expectedVersion}; received ${installedVersion}`,
		);
	}
	...
	const publicSpecifiers = publicExportKeys.map((exportKey) =>
		exportKey === "."
			? canonicalPackageName
			: `${canonicalPackageName}${exportKey.slice(1)}`,
	);
...
`,
	...
	assert(
		viteOutputFiles.includes("index.html"),
		"Vite consumer must emit index.html",
	);
	assert(viteJavaScript.length > 0, "Vite consumer must emit JavaScript");
	assert(
		viteCss.length > 0,
		"Vite consumer must emit the imported public stylesheet",
	);
	const bundledCss = (
		await Promise.all(
			viteCss.map((path) => readFile(join(viteOutputDirectory, path), "utf8")),
		)
	).join("\n");
	assert(
		...
		JSON.stringify(
			{
				packages: [
					...
					`@lemn-ltd/brand-runtime@${brandRuntimePackage.version}`,
