...
function assetPathFromHtml(html: string): string {
	...
	assert(assetPath, "Portal index.html does not reference a built asset");
	return assetPath;
}

export async function assertPortalAssetLayout(portalRoot: string): Promise<{
	assetPath: string;
	clientRoot: string;
}> {
	...
	assert(
		...
		"Portal assets are nested under dist/client/client",
	);

	const indexPath = resolve(clientRoot, "index.html");
	assert(
		await exists(indexPath),
		"Portal index.html must exist directly under dist/client",
	);
	...
	assert(
		...
		"Portal index.html references an asset outside dist/client",
	);
	assert(
		await exists(assetFile),
		`Portal index.html references missing asset ${assetPath}`,
	);
...
async function main(): Promise<void> {
	...
	const layout = await assertPortalAssetLayout(portalRoot);
	...
	try {
		...
		assert(home.status === 200, `Portal home returned HTTP ${home.status}`);
		const html = await home.text();
		assert(html.includes('id="root"'), "Portal home is not the built SPA");
		const servedAssetPath = assetPathFromHtml(html);
		assert(
			servedAssetPath === layout.assetPath,
			"Wrangler served an unexpected portal asset path",
		);
		const asset = await fetch(`${origin}${servedAssetPath}`);
		assert(
			asset.status === 200,
			`Portal asset returned HTTP ${asset.status}`,
		);
		assert(
			(await asset.arrayBuffer()).byteLength > 0,
			"Portal asset is empty",
		);
		console.log(
			"Portal Wrangler smoke passed: health=200 home=200 asset=200",
		);
	} finally {
		stopProcess(child);
		await Promise.race([
