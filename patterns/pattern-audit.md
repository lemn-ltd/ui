...
| LOCAL-PIPELINE-ADOPTION-2026-07-21 | 2026-07-21 | Repository tooling, cumulative local validation profiles, tracked hooks, GitHub validation reuse, and manual-only production promotion | **IMPLEMENTED; VERIFIED LOCALLY** — `pnpm validate:full` passed including 351 Playwright scenarios and both Worker dry-runs; `pnpm audit --prod` found no known vulnerabilities. Commands and boundaries are documented in `docs/operations/local-pipeline.md`; exact-commit evidence is written by `pnpm pipeline:local` to ignored `tooling/artifacts/checks/local-pipeline.json` |
