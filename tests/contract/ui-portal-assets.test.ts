...
import {
	assertPortalAssetLayout,
	wranglerLocalArguments,
} from "../../scripts/release/ui-portal-assets-smoke.ts";
...
test("accepts the canonical Wrangler asset root", async () => {
	...
	try {
		const layout = await assertPortalAssetLayout(directory);
...
test("rejects the Vite/Cloudflare dist/client/client nesting regression", async () => {
	...
	try {
		await assert.rejects(
			assertPortalAssetLayout(directory),
...
test("Vite and Wrangler share one canonical dist/client asset root", async () => {
	const viteConfig = await readFile(
		resolve(root, "apps/ui-portal/vite.config.ts"),
		"utf8",
	);
	const wranglerConfig = await readFile(
		resolve(root, "apps/ui-portal/wrangler.jsonc"),
		"utf8",
	);
...
test("local smoke runs Wrangler from the Portal package with the production-only config", () => {
	assert.deepEqual(wranglerLocalArguments(6543).slice(0, 12), [
		"--dir",
		"apps/ui-portal",
		"exec",
		"wrangler",
