...
import playwrightConfig from "../../apps/showcase/playwright.config.ts";
import {
...
} from "../../apps/showcase/tests/helpers/component-catalog.ts";
...
async function componentRoutesFromPageFiles(): Promise<string[]> {
	const areas = ["core", "agents"] as const;
	const routes = await Promise.all(
		areas.map(async (area) => {
			const entries = await readdir(
				resolve(showcaseRoot, `src/client/pages/${area}/components`),
				{ withFileTypes: true },
			);
			return entries
				...
				.map(
					(entry) =>
						`/${area}/components/${entry.name.slice(0, -".page.tsx".length)}`,
				);
		}),
	);
...
test("component accessibility cases exactly cover every catalog route and theme", async () => {
	...
	assert.deepEqual(await componentRoutesFromPageFiles(), catalogRoutes);
	assert.equal(
