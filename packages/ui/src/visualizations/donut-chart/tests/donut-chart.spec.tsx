...
describe("DonutChart", () => {
	...
	it("treats a zero total as an explicit empty state", () => {
		const { getByRole } = render(
			<DonutChart
				aria-label="Traffic sources"
				data={[{ label: "Direct", value: 0 }]}
			/>,
		);
		expect(getByRole("status").textContent).toContain("No non-zero data");
	});

	it("supports pie geometry without rendering a center label", () => {
		const { queryByText } = render(
			<DonutChart
				aria-label="Traffic sources"
				centerLabel="Hidden in pie"
				data={[{ label: "Direct", value: 60 }]}
				variant="pie"
			/>,
		);
		expect(queryByText("Hidden in pie")).toBeNull();
	});
});
