  ...
  return (
    ...
    >
      <ResponsiveContainer height="100%" width="100%">
        <ComposedChart accessibilityLayer data={data} margin={{ bottom: 2, left: 2, right: 2, top: 2 }}>
          {kind === 'line' ? (
            <Line {...shared} dot={false} stroke={resolvedColor} strokeWidth={2} type="monotone" />
          ) : null}
          {kind === 'area' ? (
            <Area {...shared} fill={resolvedColor} fillOpacity={0.16} stroke={resolvedColor} strokeWidth={2} type="monotone" />
          ) : null}
          {kind === 'bar' ? <Bar {...shared} fill={resolvedColor} radius={[2, 2, 0, 0]} /> : null}
          {showTooltip ? (
            <Tooltip
              contentStyle={{
