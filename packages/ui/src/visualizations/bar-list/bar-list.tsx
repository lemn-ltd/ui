...
import { chartAccessibleName, chartColor, type ChartAccessibleName, type ChartColor, type ChartStateProps } from '../internal/chart-types.js';
import './bar-list.css';
        ...
        {items.map((item, index) => {
          ...
          const content = (
            <>
              <span
                ...
                style={{ backgroundColor: chartColor(item.color, index), transform: `scaleX(${ratio})` }}
              />
          ...
          return (
            <li className="ui-bar-list__item" key={item.label}>
              {item.href ? (
                <a className="ui-bar-list__content" href={item.href}>{content}</a>
              ) : item.onSelect ? (
                <button className="ui-bar-list__content" onClick={item.onSelect} type="button">{content}</button>
              ) : (
