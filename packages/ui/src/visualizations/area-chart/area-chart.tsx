  ...
  return (
    ...
    >
      <ResponsiveContainer height="100%" width="100%">
        <RechartsAreaChart accessibilityLayer data={data} margin={{ bottom: 8, left: 4, right: 12, top: 8 }}>
          <defs>
            {series.map((item, seriesIndex) => {
              const color = chartColor(item.color, seriesIndex);
              return (
                <linearGradient id={`${gradientSeed}-${seriesIndex}`} key={item.dataKey} x1="0" x2="0" y1="0" y2="1">
          ...
          {series.map((item, seriesIndex) => {
            ...
            return (
              <Area
                ...
                fill={fill === 'gradient' ? `url(#${gradientSeed}-${seriesIndex})` : color}
