...
export interface ComboChartSeries<TDatum extends ChartDatum> extends ChartSeries<TDatum> {
  ...
  return (
    ...
    >
      <ResponsiveContainer height="100%" width="100%">
        <ComposedChart accessibilityLayer data={data} margin={{ bottom: 8, left: 4, right: hasSecondaryAxis ? 4 : 12, top: 8 }}>
          {showGrid ? <CartesianGrid stroke="var(--chart-grid)" strokeDasharray="3 3" vertical={false} /> : null}
          <XAxis dataKey={index} stroke="var(--chart-axis)" tickLine={false} />
          <YAxis stroke="var(--chart-axis)" tickLine={false} width={44} yAxisId="primary" />
          {hasSecondaryAxis ? (
            <YAxis orientation="right" stroke="var(--chart-axis)" tickLine={false} width={44} yAxisId="secondary" />
          ) : null}
          {showTooltip ? (
            <Tooltip
              contentStyle={{
          ...
          {series.map((item, seriesIndex) => {
            ...
            return item.kind === 'bar' ? (
              <Bar {...shared} fill={color} key={item.dataKey} radius={[4, 4, 0, 0]} />
            ) : (
              <Line {...shared} dot={false} key={item.dataKey} stroke={color} strokeWidth={2} type="monotone" />
            );
          })}
        </ComposedChart>
      </ResponsiveContainer>
