...
const VARIANTS: ProgressBarVariant[] = ['determinate', 'indeterminate', 'route'];
...
describe('ProgressBar', () => {
  ...
  it('defaults to the determinate variant', () => {
    ...
    expect(container.querySelector('.ui-progress-bar')?.getAttribute('data-variant')).toBe(
      'determinate',
    );
  });
  ...
  it('exposes progressbar ARIA on the determinate variant', () => {
    const { container } = render(<ProgressBar value={60} variant="determinate" />);
  ...
  it('clamps out-of-range values', () => {
    const { container } = render(<ProgressBar value={150} variant="determinate" />);
    expect(container.querySelector('.ui-progress-bar')?.getAttribute('aria-valuenow')).toBe('100');
  });
  ...
  it('omits progressbar ARIA on the indeterminate and route variants', () => {
    for (const variant of ['indeterminate', 'route'] as ProgressBarVariant[]) {
      const { container, unmount } = render(<ProgressBar variant={variant} />);
      expect(container.querySelector('.ui-progress-bar')?.getAttribute('role')).toBeNull();
      unmount();
    }
  });
  ...
  it('renders the percent label only when showLabel is set on determinate', () => {
    ...
    expect(withLabel.querySelector('.ui-progress-bar__label')?.textContent).toBe('42%');

    const { container: withoutLabel } = render(<ProgressBar value={42} variant="determinate" />);
