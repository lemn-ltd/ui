...
describe("Slider", () => {
	...
	it("renders one accessible thumb and formatted value", () => {
		...
		expect(
			getByRole("slider", { name: "Volume" }).getAttribute("aria-valuetext"),
		).toBe("40%");
		expect(getByText("40%")).toBeTruthy();
	});

	it("keeps uncontrolled visible and assistive values synchronized after keyboard input", () => {
	...
	it("supports two controlled thumbs and keyboard changes", () => {
		...
		fireEvent.keyDown(getByRole("slider", { name: "Minimum price" }), {
			key: "ArrowRight",
		});
