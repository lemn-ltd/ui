...
import { VariantsGallery, variantsFromEnum } from '../variants-gallery.js';
...
describe('variantsFromEnum', () => {
  ...
  it('maps an enum to one VariantSpec per member with matching labels', () => {
    const variants = variantsFromEnum(Statuses, (value) => <span>{value}</span>);
  ...
  it('feeds VariantsGallery one cell per enum member', () => {
    render(<VariantsGallery items={variantsFromEnum(Statuses, (value) => <span>{value}</span>)} />);
