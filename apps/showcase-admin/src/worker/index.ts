...
export async function handleShowcaseAdminRequest(
	...
	try {
		identity = await accessIdentity(request, env, verifyAccess);
	} catch {
		return secure(
			problem({
				status: 401,
	...
	if (env.DEPLOYMENT_ENVIRONMENT === "production" && !identity) {
		return secure(
			problem({
				...
				title: "Cloudflare Access required",
				detail:
					"The production Admin requires authenticated Cloudflare Access headers.",
	...
	if (identity?.kind === "service" && !isHealthRequest) {
		return secure(
			problem({
				status: 403,
				title: "Service token capability denied",
				detail: "The deployment service token may only verify Admin health.",
