	...
	return (
		<section className="admin-panel" aria-labelledby="registry-title">
			...
			<div className="admin-table-wrap" role="region" tabIndex={0}>
				<table>
					<thead><tr><th>Capability</th><th>Provider / mapping</th><th>Exact upstream</th><th>Ingestion evidence</th><th>License</th><th>Patches</th><th>Status</th></tr></thead>
					<tbody>
						{model.capabilities.map((capability) => (
							<tr key={capability.capabilityId}>
								...
								<td><strong>{capability.ingestionMode}</strong>{capability.snapshot ? <small>{capability.snapshot.selectedSourcePaths.join(", ")} · {capability.snapshot.closure.length} captured files</small> : <small>{capability.dependencyMode}</small>}</td>
