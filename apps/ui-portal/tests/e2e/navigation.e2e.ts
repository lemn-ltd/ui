...
test("canonical index and detail deep links restore every public catalog family", async ({
	...
	const deepLinks = [
		...
		["/blocks", "Purpose-built compositions"],
		["/blocks/dashboard-overview", "Dashboard overview"],
		["/patterns", "Patterns"],
		["/patterns/dashboard", "Dashboard"],
		["/providers", "One provider of record per capability"],
...
test("the single catalog shell exposes only enabled Core navigation", async ({
	...
	await expect(sidebar.getByText(/Agent activity line/i)).toHaveCount(0);
	await expect(sidebar.getByText(/Agent session/i)).toHaveCount(0);
});

test("retired and guessed disabled-area routes use the regular not-found state", async ({
	page,
}) => {
	const retiredRoutes = [
		["", "catalog"].join("/"),
		["", "brand-studio"].join("/"),
		["", "core", "components", "button"].join("/"),
		["", "agents", "components", "approval-card"].join("/"),
		["", "agents", "patterns", "agent-session"].join("/"),
	];

	for (const route of retiredRoutes) {
		await test.step(route, async () => {
			await gotoStable(page, route, { expectedSurface: "not-found" });
			await expect(page.locator(".portal-not-found")).toBeVisible();
			await expect(
...
test("route readiness rejects false operational surfaces", async ({ page }) => {
	const missingRoute = "/components/does-not-exist";
	...
	expect((failedOutcome.error as Error).message).toMatch(
		/GET .*badge\.page-[^/]+\.js \(net::ERR_FAILED\)/u,
	);
	await page.unroute(/\/assets\/badge\.page-[^/]+\.js(?:\?.*)?$/u);

	await gotoStable(page, "/");
	await page.setViewportSize({ width: 375, height: 812 });
	await page.locator("#root").evaluate((root) => {
		root.innerHTML = [
			'<section class="portal-route-error" role="alert">',
			"<h1>Page unavailable</h1>",
			"<p>The portal could not render this route.</p>",
			`<p class="portal-route-error__detail">RouteImportError:${"unbroken".repeat(32)}</p>`,
			'<button type="button">Reload page</button>',
			"</section>",
		].join("");
	});
	const errorSurface = page.locator(".portal-route-error");
