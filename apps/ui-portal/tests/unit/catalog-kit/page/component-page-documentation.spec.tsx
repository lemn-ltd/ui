...
describe("ComponentPage documentation contract", () => {
	...
	it("adapts catalog pages to the common interactive reference layout", () => {
		...
		expect(screen.getByText(UI_PACKAGE_INSTALL_COMMAND)).toBeDefined();
		expect(UI_PACKAGE_INSTALL_COMMAND).toMatch(
			/^pnpm add @lemn-ltd\/ui@\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u,
		);
		expect(container.querySelectorAll(".portal-example")).toHaveLength(2);
		expect(container.textContent).toContain(
			"import { Button } from '@lemn-ltd/ui';",
		);
		expect(container.textContent).not.toMatch(LEGACY_UI_PACKAGE_PATTERN);
		const hero = container.querySelector(".portal-example");
	...
	it("keeps pattern pages on their original page chrome", () => {
		...
		expect(document.querySelector(".portal-docs-page")).toBeNull();
	});
});
