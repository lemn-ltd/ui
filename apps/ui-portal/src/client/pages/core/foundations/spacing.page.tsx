import { FoundationPage } from '@portal/catalog-kit';
import { PageSection } from '@lemn-ltd/ui';
import type { ReactElement } from 'react';

const STEPS: { step: number; px: number }[] = [
  { step: 0, px: 0 },
  { step: 1, px: 4 },
  { step: 2, px: 8 },
  { step: 3, px: 12 },
  { step: 4, px: 16 },
  { step: 5, px: 20 },
  { step: 6, px: 24 },
  { step: 8, px: 32 },
  { step: 10, px: 40 },
  { step: 12, px: 48 },
  { step: 16, px: 64 },
];

const RADII = [
  { label: 'sm', cssVar: '--lemn-radius-small' },
  { label: 'md', cssVar: '--lemn-radius-medium' },
  { label: 'lg', cssVar: '--lemn-radius-large' },
  { label: 'pill', cssVar: '--radius-pill' },
];

function SpacingPage(): ReactElement {
  return (
    <FoundationPage
      caption="A non-contiguous spacing scale (7, 9, 11, 13–15 are intentionally absent) and four radii."
      title="Spacing & radii"
    >
        <PageSection title="Space">
          {STEPS.map((entry) => (
            <div className="portal-space-row" key={entry.step}>
              <span className="portal-space-label">
                space-{entry.step} · {entry.px}px
              </span>
              <span
                className="portal-space-bar"
                style={{ width: `var(--lemn-space-${entry.step})` }}
              />
            </div>
          ))}
        </PageSection>

        <PageSection title="Radii">
          <div className="portal-radii-grid">
            {RADII.map((radius) => (
              <div className="portal-radii-cell" key={radius.label}>
                <span
                  className="portal-radii-chip"
                  style={{ borderRadius: `var(${radius.cssVar})` }}
                />
                <span className="portal-space-label">{radius.label}</span>
              </div>
            ))}
          </div>
        </PageSection>
    </FoundationPage>
  );
}

export default SpacingPage;
