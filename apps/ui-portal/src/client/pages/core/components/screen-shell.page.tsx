...
// ScreenShell fills the dynamic viewport (with a 100vh fallback), so it renders
// inside a fixed-height bordered frame
...
function ScreenShellPage(): ReactElement {
	return (
		<ComponentPage
			status="stable"
			summary="The app frame: a fixed sidebar beside a main column that holds an optional top bar over a scrolling content region. It follows the dynamic viewport height; only the content region scrolls."
			title="Screen shell"
		>
			<ExampleBlock
				code={`<ScreenShell
						...
						>
							<div style={contentStyle}>
								<Card title="Content region">
									The main column holds an optional top bar above this scrolling
									content area.
								</Card>
							</div>
						</ScreenShell>
				...
				rows={[
					{
						...
						type: "ReactNode",
						description:
							"The fixed-width rail rendered in the first grid column.",
					},
					{
						name: "children",
						type: "ReactNode",
					...
					},
					{
						...
						type: "ReactNode",
						description:
							"Optional bar pinned above the content region; omitted when not provided.",
					},
				]}
			/>
		</ComponentPage>
