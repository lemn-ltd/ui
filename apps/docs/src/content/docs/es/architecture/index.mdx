...
La fuente es `BrandingDefinition` v1. Publicacion la compila a un objeto
inmutable firmado; runtime entrega solo un mode verificado. Los componentes no
parsean el JSON fuente.

## Responsabilidades

| Limite | Posee | No posee |
| --- | --- | --- |
| `@lemn-ltd/brand-contract` | Schema, validation, compiler, diagnostics, envelope, tokens, provider themes | Persistence, auth, publicacion, seleccion activa |
...
| Showcase/Admin | Evidencia publica y experimentos/proposals protegidos | Estado durable o activacion |
...
@lemn-ltd/brand-contract -> signed object -> AgentOps private runtime
@lemn-ltd/brand-studio -> typed intents ----^            |
@lemn-ltd/brand-runtime <- server consumer --------------+
```

Postgres es autoridad de estado; R2 privado almacena objetos
content-addressed; caches por hash solo aceleran. Un Workspace tiene un Branding
y multiples BrandingVersions. Solo un humano activa una version publicada.
Light/dark son modes completos de la misma identidad, no familias de branding.
