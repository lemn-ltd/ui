...
test('Dialog opens, traps Escape, and returns focus to its trigger', async ({ page }) => {
  await gotoStable(page, '/core/components/dialog');

  const trigger = page.locator('.showcase-example__preview button').first();
  await trigger.click();
...
test('Menu opens, roves focus across items with arrow keys, and closes on Escape', async ({
  ...
  await gotoStable(page, '/core/components/menu');

  await page.locator('.showcase-example__preview button').first().click();
...
test('Combobox filters its options and shows a no-matches state', async ({ page }) => {
  await gotoStable(page, '/core/components/combobox');
...
test('Accordion expands a collapsed item', async ({ page }) => {
  await gotoStable(page, '/core/components/accordion');
