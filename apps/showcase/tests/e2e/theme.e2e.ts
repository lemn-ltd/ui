...
function bodyBackground(page: import('@playwright/test').Page): Promise<string> {
...
test('the theme toggle flips data-theme and the token-bound background', async ({ page }) => {
  ...
  await expect.poll(() => bodyBackground(page)).toBe('rgb(6, 8, 12)');
