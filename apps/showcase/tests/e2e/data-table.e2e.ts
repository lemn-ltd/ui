...
test.describe('Data table', () => {
  ...
  test('select-all reveals the bulk actions bar over 137 rows', async ({ page }) => {
    ...
    await expect(bulk.locator('.ui-data-table__bulk-count')).toHaveText('137 selected');
  });

  test('paginates across the 137-row dataset', async ({ page }) => {
    const status = page.locator('.ui-data-table__page-status').first();
