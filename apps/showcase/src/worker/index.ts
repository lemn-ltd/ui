...
export default class UiShowcaseWorker extends WorkerEntrypoint<UiShowcaseEnv> {
  override async fetch(request: Request): Promise<Response> {
    ...
    if (pathname === '/_status' || pathname === '/_status.json' || pathname === '/health/deep') {
...
function protectedStatusResponse(request: Request, env: UiShowcaseEnv): Response {
...
function llmsResponse(): Response {
  return new Response(
    [
      '# Appranks UI',
      '',
      'Use @appranks/ui as the official company design-system package.',
      ...
      '- If a reusable component is missing, add it to @appranks/ui and document it in the showcase.',
    ].join('\n'),
...
function llmsFullResponse(): Response {
  const lines = componentCatalog.flatMap((entry) => [
    ...
    `status: ${entry.status}`,
    `intent: ${entry.intent}`,
    `import: import { ${componentNameFromSlug(entry.slug)} } from '@appranks/ui';`,
    '',
  ]);

  return new Response(['# Appranks UI Component Catalog', '', ...lines].join('\n'), {
    headers: { 'content-type': 'text/plain; charset=utf-8' },
  });
}
