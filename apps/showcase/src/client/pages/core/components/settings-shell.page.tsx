...
function renderSection(id: string): ReactNode {
  if (id === 'general') {
    return (
      <>
        <PageSection caption="How sessions are classified and surfaced." title="General">
          <Card>
            <SettingsRow
            ...
            <SettingsRow description="Pick the language for the interface." label="Language">
              <InputSelect
                aria-label="Language"
                defaultValue="en"
                options={[
            ...
            <SettingsRow description="Density of list and table rows." label="Display density">
              <SegmentedControl
        ...
        <PageSection caption="Long content scrolls within the detail pane." title="Telemetry">
          <Card>
            {[
              'Crash reports',
              'Usage analytics',
              ...
              <SettingsRow description={`Toggle ${label.toLowerCase()}.`} key={label} label={label}>
  ...
  if (id === 'appearance') {
    return (
      <PageSection caption="Theme and motion preferences." title="Appearance">
        <Card>
          <SettingsRow description="Match the system or pick a fixed theme." label="Theme">
            <SegmentedControl
              aria-label="Theme"
              defaultValue="system"
              segments={[
          ...
          <SettingsRow description="Reduce non-essential animation." label="Reduced motion">
  ...
  return (
    ...
    >
      <Card>
        <p>Each section owns its own controls; the shell only routes between them.</p>
      </Card>
    </PageSection>
  );
}
  ...
  {(section) => renderSection(section)}
        ...
        render={() => (
          <div style={{ height: 420 }}>
            <SettingsShell collapsibleNav defaultNavCollapsed groups={GROUPS} variant="surface">
              {(section) => renderSection(section)}
            </SettingsShell>
          </div>
        )}
...
>
  {(section) => renderSection(section)}
      ...
      <ExampleBlock
        code={`<SettingsShell variant="surface" searchable={false} groups={groups}>
  {(section) => renderSection(section)}
</SettingsShell>`}
        render={() => (
          <div style={{ height: 360 }}>
            <SettingsShell groups={GROUPS} searchable={false} variant="surface">
              {(section) => renderSection(section)}
            </SettingsShell>
          </div>
        )}
        ...
        rows={[
          {
            ...
            description: 'Grouped sections; each group has an optional header and a list of items.',
          },
          {
            name: 'children',
          ...
          },
          {
            ...
            type: '(id: string) => void',
            description: 'Called when the active section changes (click or keyboard).',
          },
          {
            name: 'title',
            type: 'ReactNode',
            description: 'Heading above the panes; also the accessible name in modal mode.',
          },
          {
            name: 'onClose',
            type: '() => void',
            description: 'Renders a header close control inline; ignored in modal mode.',
          },
          {
            name: 'searchable',
            type: 'boolean',
          ...
          },
          {
            ...
            description: 'Control the search query externally; omit for built-in filtering.',
          },
          {
            name: 'searchEmpty',
            type: 'ReactNode',
          ...
          },
          {
            ...
            type: 'ReactNode',
            description: 'Sticky regions above/below the scrolling detail body.',
          },
          {
            name: 'error',
            type: 'ReactNode',
          ...
          },
          {
            name: 'variant',
            type: "'plain' | 'surface'",
            description: "'plain' fills a region; 'surface' adds a bordered, rounded card frame.",
          },
          {
