...
> {
  ...
  public override render(): ReactNode {
    if (this.state.failed) {
      return <span className="showcase-live-preview__status">Preview unavailable</span>;
    }
...
function calculateFrame(boundary: HTMLElement, scale: number): {
  readonly x: number;
  readonly y: number;
...
export function LivePreview({ entry }: { readonly entry: UiShowcaseEntry }): ReactElement {
  const viewportRef = useRef<HTMLDivElement>(null);
  const canvasRef = useRef<HTMLDivElement>(null);
  const scaleRef = useRef(1);
  const [active, setActive] = useState(false);
  const [ready, setReady] = useState(false);
  const [transform, setTransform] = useState<PreviewTransform>({ x: 0, y: 0, scale: 1 });
  ...
  useEffect(() => {
    ...
    const measure = (): void => {
      ...
      frame = requestAnimationFrame(() => {
        const boundary = canvas.querySelector<HTMLElement>('[data-showcase-preview-content]');
      ...
      {active ? (
        ...
        >
          <PreviewErrorBoundary>
            <Suspense
              fallback={<span className="showcase-live-preview__status">Loading live preview…</span>}
            >
